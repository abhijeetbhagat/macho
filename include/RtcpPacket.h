...
struct RtcpPacket {
  ...
  uint16_t packet_len;
  uint32_t ssrc;
  RtcpPacket(int version, int padding, int reception_report_cnt,
             int packet_type, uint16_t packet_len, uint32_t ssrc)
      : version(version), padding(padding),
        reception_report_cnt(reception_report_cnt), packet_type(packet_type),
        packet_len(packet_len), ssrc(ssrc) {}
