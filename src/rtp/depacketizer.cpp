...
RtpPacket Depacketizer::parse_rtp(const std::string &buf) {
  ...
  auto seq_num =
      uint16_t((uint16_t)payload[2] << (uint16_t)8 | (uint16_t)payload[3]);
  auto timestamp = (uint32_t)payload[4] << 24 | (uint32_t)payload[5] << 16 |
                   (uint32_t)payload[6] << 8 | (uint32_t)payload[7];
  auto ssrc = (uint32_t)payload[8] << 24 | (uint32_t)payload[9] << 16 |
  ...
  if (cc > 0) {
    ...
    for (uint32_t i = 0; i < cc; i++) {
      uint32_t csrc = payload[j] << 24 | payload[++j] << 16 |
                      payload[++j] << 8 | payload[++j];
  ...
  std::copy(payload.data() + j, payload.data() + payload.size(),
            packet.header.raw_data.get());
...
RtcpPacket Depacketizer::parse_rtcp(const std::string &data) {
  ...
  return RtcpPacket{.version = version,
                    .padding = padding,
